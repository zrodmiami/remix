...
test("route module functions are passed expected arguments", async ({
  ...
  let expectedDataArgs = ["matches", "params", "request"];
  expect(actionArgs.server.sort()).toEqual(
    ["context", ...expectedDataArgs].sort()
  );
  expect(actionArgs.client.sort()).toEqual(
    ["location", "serverAction", ...expectedDataArgs].sort()
  );
  expect(loaderArgs.server.sort()).toEqual(
    ["context", ...expectedDataArgs].sort()
  );
  expect(loaderArgs.client.sort()).toEqual(
    ["location", "serverLoader", ...expectedDataArgs].sort()
  );
  ...
  expect(metaArgs).toEqual(["data", "error", "location", "matches", "params"]);
});
